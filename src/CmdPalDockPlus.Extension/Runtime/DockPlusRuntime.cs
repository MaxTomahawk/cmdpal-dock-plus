...
internal sealed class DockPlusRuntime : IAsyncDisposable
{
    public DockPlusRuntime()
    {
        ...
        Coordinator = new DockCoordinator(ProfileStore, Tracker, Activator, Launcher);
    }
    ...
    public async ValueTask DisposeAsync() => await Coordinator.DisposeAsync().ConfigureAwait(false);
}
