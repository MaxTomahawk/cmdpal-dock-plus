...
internal sealed class DockCoordinator : IAsyncDisposable
{
    ...
    public DockCoordinator(IProfileStore profileStore, WindowTracker tracker, WindowActivator activator, AppLauncher launcher)
    {
    ...
    public async Task ActivateTileAsync(TileIdentity identity)
    {
        ...
        {
            await _activator.FocusAsync(window, default).ConfigureAwait(false);
        }
    }
    ...
    public async Task RunUserActionAsync(TileIdentity identity, string actionId)
    {
        ...
        {
            await _launcher.RunUserActionAsync(action, default).ConfigureAwait(false);
        }
    }
    ...
    private async Task RefreshAsync()
    {
        ...
        try
        {
            ...
            {
                var matching = _tracker.Snapshot
                    .Where(window => Matches(profile, window))
                    .Select(ToTileWindow)
                    .ToArray();
                states.AddRange(_composer.Compose(profile, matching));
            }
            ...
            foreach (var state in states)
            {
                ...
                if (_items.TryGetValue(state.Identity, out var existing))
                {
                    existing.Update(state);
                }
                else
                {
                    _items.Add(state.Identity, new DockTileListItem(this, state));
                }
            }
    ...
    private static bool Matches(AppProfile profile, WindowSnapshot window)
    {
        ...
        {
            var configured = Path.GetFileName(profile.Application.ExecutablePath);
            return string.Equals(configured, window.ExecutableName, StringComparison.OrdinalIgnoreCase);
        }

        return false;
    }

    private static TileWindow ToTileWindow(WindowSnapshot window)
    {
        ...
        return new TileWindow(window.Hwnd, window.ProcessId, window.Title, window.IsActive, window.MruRank, values);
    }
}
